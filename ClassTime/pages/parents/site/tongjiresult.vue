			...
			<view class="bg">
				<view class="qiun-columns">
						<view class="qiun-bg-white qiun-title-bar qiun-common-mt" >
							<view class="qiun-title-dot-light">{{tips}}</view>
...
<script>
	...
	export default {	
		...
		onLoad(options) {
			...
			//相关数据
			_self.dateid = options['dateid'];    //日期
			_self.cid = options['cid'];     //老师id
			_self.child_id = options['child_id'];       //类型
			_self.headermsg = '统计结果,Statistics';
		},
		onReady() {
			_self.getServerData();
		},
		methods: {
			...
			getServerData() {
				let ret = _self.getUserInfo();				
					_self.sendRequest({
				        url : _self.GetChildStatisticsUrl,
				        method : _self.Method,
				        data : {
							"token":ret.token,
							"guid":ret.guid,
			...
			fillData(data) {
				...
				_self.tips = data.tips;
				_self.sliderMax = data.Candle.categories.length;
